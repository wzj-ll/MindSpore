        ...
        for i, data_2d in enumerate(lpe_2d):
            for j, data in enumerate(data_2d):
                ...
                if lpe_names[i] == "error":
                    img = ax.imshow(data.T, vmin=0, vmax=1, cmap=plt.get_cmap("jet"), origin='lower') # type: ignore
                else:
                    img = ax.imshow(data.T, vmin=vmin_list[j], vmax=vmax_list[j], cmap=plt.get_cmap("jet"), origin='lower') # type: ignore
